...
def _parse_json_message(result):
    """Read a JSON object from Ollama even if a model wrapped it in a code fence."""
...
def plan_local(ai, model, topic, audience, tone, count):
    ...
    instructions = (
        ...
        'Visual prompts describe camera, action, lighting and a consistent style. Do not put captions or text into the generated visuals. '
    ...
    # Qwen 3/3.5 are reasoning-capable models. For strict JSON jobs we explicitly
    # disable thinking so the response budget is used for the actual JSON output.
    result = ai.request('/api/chat', payload, timeout=300)
    data = _parse_json_message(result)

    # A local retry is safe and free of cloud/API charges. It makes storyboard
    # generation resilient against the occasional empty/non-JSON model response.
    if data is None:
        retry = dict(payload)
        retry['messages'] = [
            {'role': 'system', 'content': instructions},
            {'role': 'user', 'content': brief},
            {'role': 'user', 'content': 'Return the requested storyboard now as one compact valid JSON object only.'}
        ]
        retry['options'] = dict(payload['options'], temperature=0.0)
        result = ai.request('/api/chat', retry, timeout=300)
        data = _parse_json_message(result)
